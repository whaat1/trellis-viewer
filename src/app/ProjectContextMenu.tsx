...
export function ProjectContextMenu({ project, className, children, ...rowProps }: {
  project: Project; className: string; children: ReactNode;
} & HTMLAttributes<HTMLDivElement>) {
  const macActions = /Mac/.test(navigator.platform);
  ...
  function requestRemove() { setPosition(null); setFailure(''); setConfirming(true); }
  function cancel() { if (!submitting.current) { setConfirming(false); focusRow(); } }

  return <div {...rowProps} ref={row} className={className} onContextMenu={event => {
    ...
    {children}
    <button aria-label={`移除项目 ${project.name}`} onClick={requestRemove}>×</button>
    {position && createPortal(<div ref={menu} className="project-context-menu" style={position} role="menu" aria-label={`${project.name}的操作`} onKeyDown={event => {
      ...
      <button role="menuitem" className="project-remove-action" onClick={requestRemove}>从列表移除</button>
      <button role="menuitem" disabled={!macActions} title={macActions ? undefined : '此操作目前仅支持 macOS'} onClick={() => void action('reveal')}>在 Finder 中显示</button>
      <button role="menuitem" disabled={!macActions} title={macActions ? undefined : '此操作目前仅支持 macOS'} onClick={() => void action('copy')}>复制路径</button>
    </div>, document.body)}
