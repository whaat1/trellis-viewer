...
import './tasks.css';
import { moveTreeItem, parseTreeOrder, treeOrderStorageKey } from '../../state/tree-ordering';
import type { TreeRow } from '../../state/model';
...
export function TaskTree() {
  const projectId = useStore(s => s.index?.projectId ?? '');
  return <ProjectTaskTree key={projectId} projectId={projectId}/>;
}
function ProjectTaskTree({ projectId }: { projectId: string }) {
  const [order, setOrder] = useState(() => { try { return parseTreeOrder(localStorage.getItem(treeOrderStorageKey(projectId))); } catch { return {}; } });
  const [orderError, setOrderError] = useState('');
  const [dropTarget, setDropTarget] = useState('');
  const dragged = useRef<TreeRow | null>(null);
  ...
  const rows = useMemo(() => tasks ? flattenTree(tasks, rootKey, docs, expanded, order) : [], [tasks, rootKey, docs, expanded, order]);
  ...
  function move(from: TreeRow, to: TreeRow) {
    if (!from.sortScope || from.sortScope !== to.sortScope) return;
    const siblings = rows.filter(row => row.sortScope === from.sortScope).map(row => row.id);
    const next = moveTreeItem(order, from.sortScope, siblings, from.id, to.id);
    if (next === order) return;
    try {
      localStorage.setItem(treeOrderStorageKey(projectId), JSON.stringify(next));
      setOrder(next); setOrderError('');
    } catch { setOrderError('目录顺序保存失败，已保留原顺序。请重试。'); }
  }
  return <aside className="task-tree pane" aria-label="子任务和文档"><header className="pane-header"><strong>任务目录</strong><span className="spacer"/><span className="subtle">{index?.tasks[rootKey]?.childKeys.length ?? 0} 个子任务</span></header>{orderError && <p role="alert">{orderError}</p>}
    <div className="tree-scroll" ref={scroll} data-perf-scroll="tree" role="tree" aria-label="任务文档树"><div style={{ position: 'relative', height: virtual.getTotalSize() }}>
      {virtual.getVirtualItems().map(item => {
        ...
        return <Row key={row.id} role="treeitem" aria-level={row.depth + 1} aria-expanded={row.expandable ? row.open : undefined} aria-selected={row.kind === 'document' ? active : undefined} className={`tree-row ${row.kind} ${active ? 'active' : ''} ${row.taskKey === rootKey && row.kind === 'task' ? 'is-root' : ''} ${completed ? 'is-completed' : ''} ${cancelled ? 'is-cancelled' : ''}`} style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: 36, paddingLeft: 14 + row.depth * 16, transform: `translateY(${item.start}px)`, boxShadow: dropTarget === row.id ? `inset 0 ${rows.findIndex(candidate => candidate.id === dragged.current?.id) < item.index ? '-2px' : '2px'} 0 #5271a8` : undefined }} title={`${hint}${row.sortScope ? ' · 拖动排序，或按 Alt+↑/↓' : ''}`} draggable={!!row.sortScope}
          ...
          onDragLeave={() => setDropTarget(current => current === row.id ? '' : current)}
          onDrop={event => { event.preventDefault(); if (dragged.current) move(dragged.current, row); dragged.current = null; setDropTarget(''); }}
          onDragEnd={() => { dragged.current = null; setDropTarget(''); }}
          onKeyDown={event => { if (!event.altKey || !row.sortScope || !['ArrowUp', 'ArrowDown'].includes(event.key)) return; event.preventDefault(); const siblings = rows.filter(candidate => candidate.sortScope === row.sortScope); const target = siblings[siblings.findIndex(candidate => candidate.id === row.id) + (event.key === 'ArrowUp' ? -1 : 1)]; if (target) move(row, target); }}
          onClick={row.kind === 'document' ? () => selectDocument(row.taskKey, row.documentKey!) : row.expandable ? () => toggleExpanded(row.kind === 'children' ? childGroupCollapseId(rootKey) : row.id, row.kind === 'task' ? row.taskKey : undefined) : undefined}>
